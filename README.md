...
1.  **Create App:** Log in to [Anvil](https://anvil.works), create a new app, enable **Uplink**, and copy the Key.
2.  **Connect Backend:**
    ```bash
    export ANVIL_UPLINK_KEY="server_YourKeyHere"
    python3 backend/anvil_service.py
    ```
3.  **Fetch Data:** In your Anvil app code, call the local function:
    ```python
    # Returns data from your local MongoDB
    data = anvil.server.call('get_recent_tles', limit=10)
    ```

For full details on available functions and architecture, see the [Anvil Integration Guide](ANVIL_INTEGRATION.md).
